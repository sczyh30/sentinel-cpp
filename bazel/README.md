# Building Sentinel with Bazel

## Production environments

To build Sentinel with Bazel in a production environment, where the [Sentinel
...
2. Configure, build and/or install the [Sentinel dependencies](https://www.envoyproxy.io/docs/envoy/latest/install/building.html#requirements).
3. `bazel build --package_path %workspace%:<path to Sentinel source tree> //source/exe:sentinel-static`
   from the directory containing your WORKSPACE.

## Quick start Bazel build for developers

As a developer convenience, a [WORKSPACE](https://github.com/alibaba/sentinel-cpp/blob/master/WORKSPACE) and
[rules for building a recent
version](https://github.com/alibaba/sentinel-cpp/blob/master/bazel/repositories.bzl) of the various Sentinel
dependencies are provided. These are provided as is, they are only suitable for development and
testing purposes. The specific versions of the Sentinel dependencies used in this build may not be
up-to-date with the latest security patches. See
[this doc](https://github.com/alibaba/sentinel-cpp/blob/master/bazel/EXTERNAL_DEPS.md#updating-an-external-dependency-version)
...
Sentinel compiles and passes tests with the version of clang installed by XCode 9.3.0:
...
5. `bazel build //source/exe:sentinel-static` from the Sentinel source directory.
...
Though Sentinel has been run in production compiled with GCC 4.9 extensively, we now require
...
# Testing Sentinel with Bazel

All the Sentinel tests can be built and run with:
...
the units tests in
[test/common/http/async_client_impl_test.cc](https://github.com/alibaba/sentinel-cpp/blob/master/test/common/http/async_client_impl_test.cc):
...
It's also possible to pass into an Sentinel test additional command-line args via `--test_arg`. For
...
Sentinel can produce backtraces on demand and from assertions and other fatal
...
Sentinel (or test) output on stdin. You can postprocess a log or pipe the output of
an Sentinel process. If passed some arguments it runs the arguments as a child
...
# Additional Sentinel build and test options
...
```
bazel build -c opt //source/exe:sentinel-static
```
...
```
bazel build --copts=-DENVOY_DEFAULT_MAX_STATS=32768 --copts=-DENVOY_DEFAULT_MAX_OBJ_NAME_LENGTH=150 //source/exe:sentinel-static
```
...
```
bazel --bazelrc=/dev/null build -c opt //source/exe:sentinel-static.stripped.stamped
```
...
https://github.com/alibaba/sentinel-cpp/issues/1000). Then run:

```
test/run_sentinel_bazel_coverage.sh
```
...
```
COVERAGE_TARGET=//test/common/common:base64_fuzz_test VALIDATE_COVERAGE=false test/run_sentinel_bazel_coverage.sh
```

# Cleaning the build and test artifacts

`bazel clean` will nuke all the build/test artifacts from the Bazel cache for
Sentinel proper. To remove the artifacts for the external dependencies run
`bazel clean --expunge`.
...
# Adding or maintaining Sentinel build rules

See the [developer guide for writing Sentinel Bazel rules](DEVELOPER.md).
