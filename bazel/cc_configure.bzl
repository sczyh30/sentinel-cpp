...
def _quiet_fake_which(program):
    return struct(_sentinel_fake_which = program)
...
def _find_cxx(repository_ctx):
    ...
    if hasattr(real_cxx, "_sentinel_fake_which"):
        real_cxx = repository_ctx.which(real_cxx._sentinel_fake_which)
        if real_cxx == None:
            find_cc(struct(
...
def _build_sentinel_cc_wrapper(repository_ctx):
    ...
    repository_ctx.template("extra_tools/sentinel_cc_wrapper", repository_ctx.attr._sentinel_cc_wrapper, {
    ...
    return repository_ctx.path("extra_tools/sentinel_cc_wrapper")

def _needs_sentinel_cc_wrapper(repository_ctx):
