...
sentinel_real_cc = {ENVOY_REAL_CC}
sentinel_real_cxx = {ENVOY_REAL_CXX}
sentinel_cflags = {ENVOY_CFLAGS}
sentinel_cxxflags = {ENVOY_CXXFLAGS}
...
def sanitize_flagfile(in_path, out_fd):
  with open(in_path, "rb") as in_fp:
    for line in in_fp:
      ...
      elif "-stdlib=libc++" in sentinel_cxxflags:
        os.write(out_fd, "-lc++\n")
...
def main():
  ...
  if sys.argv[1:5] == ["-E", "-xc++", "-", "-v"]:
    os.execv(sentinel_real_cxx, [sentinel_real_cxx] + sys.argv[1:] + shlex.split(sentinel_cxxflags))

  # Detect if we're building for C++ or vanilla C.
  if any(map(is_cpp_flag, sys.argv[1:])):
    compiler = sentinel_real_cxx
    # Append CXXFLAGS to all C++ targets (this is mostly for dependencies).
    argv = shlex.split(sentinel_cxxflags)
  else:
    compiler = sentinel_real_cc
    # Append CFLAGS to all C targets (this is mostly for dependencies).
    argv = shlex.split(sentinel_cflags)
  ...
  if "-static-libstdc++" in sys.argv[1:] or "-stdlib=libc++" in sentinel_cxxflags:
    for arg in sys.argv[1:]:
      if arg == "-lstdc++":
        if "-stdlib=libc++" in sentinel_cxxflags:
          argv.append("-lc++")
