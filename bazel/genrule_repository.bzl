def _genrule_repository(ctx):
    ...
    genrule_cmd_file = Label("@sentinel//bazel").relative(str(ctx.attr.genrule_cmd_file))
    ctx.symlink(genrule_cmd_file, "_sentinel_genrule_cmd.genrule_cmd")
    cat_genrule_cmd = ctx.execute(["cat", "_sentinel_genrule_cmd.genrule_cmd"])
